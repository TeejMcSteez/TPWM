...
ipcMain.handle('add-new-info', (event, userInfo) => {
    try {
        ...
        const encryptedUsername : string = encryptor.AES.encrypt(userInfo[0], key).toString();
        const encryptedPassword : string = encryptor.AES.encrypt(userInfo[1], key).toString();
        const encryptedWebsite : string = encryptor.AES.encrypt(userInfo[2], key).toString();
        data.push({ Username: encryptedUsername, Password: encryptedPassword, Website: encryptedWebsite });
